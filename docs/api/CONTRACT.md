...
Every route also shares the same call-time failure rule (fixed 2026-07-21 across all of them — a real gap found via live testing, not by inspection alone: several routes only wrapped credential loading in `try/except OpenAiError`, so a genuine failure from the actual OpenAI request — network error, provider outage, a key that loads but is rejected at call time — propagated as an unhandled `500` instead of the documented envelope). Any `OpenAiError` raised by the request itself now returns `502 ai_call_failed` with the underlying message, distinct from the `503 openai_not_configured` case above (missing/unloadable credentials vs. a credentialed request that failed).
